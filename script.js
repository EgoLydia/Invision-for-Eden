...
const expandedfoodItem = document.getElementById("expanded-food-item");

foodItemModal.addEventListener("click", closeFoodItemModal);

function onFoodItemClick(e) {
  const foodItem = e.currentTarget;
  foodItemModal.style.display = "flex";
  expandedfoodItem.src = foodItem.children[0].src;
}
...
async function onViewMealClick(e) {
  ...
  [...modalContentClone.children[0].children].forEach((child) =>
    child.addEventListener("click", onFoodItemClick)
  );
  ...
  viewMealCardClone.children[0].children[1].addEventListener(
    "click",
    async () => {
